...
library(gganimate)
map_anim <- SouthAmericasf %>% left_join(questions_over_time %>% mutate(PAIS = stringr::str_to_lower(PAIS)),by = c('name' = 'PAIS')) %>% 
  ggplot() +
  geom_sf(aes(fill = percent_good_ls3)) +
  theme_map() +
  theme(legend.background = element_blank()) +
  theme(legend.position = "right") + transition_states(YEAR)
